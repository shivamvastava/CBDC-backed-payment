...
contract SimpleDeployScript is Script {
    ...
    function run() external {
        ...
        require(
            winrContract.transfer(conversionService, CONVERSION_SERVICE_BALANCE), "SimpleDeploy: WINR transfer failed"
        );
    ...
    function _saveDeploymentAddresses() internal {
        string memory deploymentInfo = string(
            abi.encodePacked(
                ...
                vm.toString(amlHook),
                "\n",
                "TOKEN_CONVERSION_SERVICE=",
                vm.toString(conversionService),
                "\n",
                "SIMPLE_POOL_FACTORY=",
                vm.toString(poolFactory),
                "\n"
            )
        );
