...
contract TokenConversionService is Ownable, ReentrancyGuard, Pausable {
    ...
    constructor(address wInr) Ownable(msg.sender) {
        require(wInr != address(0), "TokenConversionService: Invalid wINR address");
        W_INR = wInr;
    }
     ...
     */
    function convertToWinr(address token, uint256 amount)
        external
        nonReentrant
        whenNotPaused
        onlyAuthorizedToken(token)
        returns (uint256 wInrAmount)
    {
        ...
        IERC20(W_INR).safeTransfer(msg.sender, wInrAmount);
